...
> **진행 방식**: Phase 0 → 1 → 2-1 ~ 2-4 까지 먼저 완성한다. Phase 2 는 embedded-server 의 Spring AI MCP 도구를 Node MCP 로 전면 이관하는 것이 목표이며, Sub-phase 2-1(인프라+시그니처+mock) → 2-2(분석+contract) → 2-3(Node 핸들러 일괄) → 2-4(본체 HTTP tool 구현 + as-is/to-be 비교 검증) 순으로 진행한다. Phase 3 은 Desktop lifecycle UX 를 구체화하고, 3-1(미실행 감지와 자동 launch) → 3-2(미설치 감지·설치 안내) 순으로 진행한다.
...
MCP tool 흐름 안에서 자동으로 진단하고 복구할 수 있게 한다. `neosql-mcp` 는 Electron
app 을 직접 소유하지 않고, 실행 상태 확인·미실행 시 detached launch·readiness
polling·설치 안내까지만 담당한다.

### Phase 3 진행 원칙

- upstream 의존 tool 호출 전 공통 `ensureDesktopReady()` 흐름을 거친다.
...
tool 호출
  → ensureDesktopReady()
    → healthcheck
    → running이면 바로 통과
    → not_running / stale_socket이면 Electron app detached launch
    → healthcheck polling
    → ready 확인
  → 원 tool 요청 실행
```
...
- 자동 launch 대상은 `not_running` 과 `stale_socket` 으로 제한한다. `timeout` 에서는
  자동 launch 하지 않고 명확한 timeout/unresponsive 응답을 반환한다.
- 원 tool 요청은 `ensureDesktopReady()` 가 통과한 뒤 1회 실행한다. 이미 upstream 에
  전달된 요청의 timeout 은 자동 재시도하지 않는다.
- 요청 저장은 영속 큐가 아니라 tool handler 내부의 in-flight 요청 보관을 의미한다.
  launch/readiness 확인 후 같은 handler 흐름에서 원 요청을 이어서 실행한다.
- Phase 3-1 은 앱이 설치되어 있다는 전제에서 "꺼져 있으면 띄운다"는 흐름을 다룬다.
  launch 대상 앱을 찾지 못하는 최소 상태는 둘 수 있지만, OS 별 정밀 설치 감지와 안내는
  Phase 3-2 로 분리한다.

### Phase 3-1 · 미실행 감지와 자동 launch

**목표**: 모든 upstream 의존 tool 이 원 요청 실행 전에 동일한 Desktop readiness 보장
흐름을 거치게 한다. `ensureDesktopReady()` 가 `not_running` 또는 `stale_socket` 을
확인하면 NeoSQL Desktop 을 detached launch 하고 readiness polling 후 원 요청을
이어서 실행한다.
...
- upstream 의존 tool 공통 wrapper 또는 `postRpc` 진입 전 readiness 보장 경계.
- in-flight 원 요청을 readiness 확인 뒤 1회 실행하는 control flow.
...
- 결과 상태: `launched`, `not_installed`, `launch_failed`, `not_ready_timeout` 등.
- stale POSIX socket 과 listener 부재의 사용자-facing UX 통합, 로그/진단 구분 유지.
- MCP host 별 수동 UX 확인 절차 (`docs/e2e-manual.md` 확장).

완료 조건:

- upstream 의존 tool 이 `ensureDesktopReady()` 를 통과한 뒤에만 upstream RPC 를 호출한다.
- `running` 은 바로 원 요청 실행으로 이어진다.
- `timeout` 은 자동 launch 로 이어지지 않고 unresponsive/timeout 응답으로 종료된다.
- 이미 upstream 에 전달된 요청의 timeout 은 자동 재시도하지 않는다.
- `not_running` / `stale_socket` 에서 detached launch 를 시도한다.
- readiness polling 성공 시 원 tool 요청을 1회 실행한다.
- readiness polling 이 timeout 되면 원 요청을 실행하지 않고 `not_ready_timeout` 을 반환한다.
