...
detached launch 요청·설치 안내까지만 담당한다.

### Phase 3 진행 원칙

- upstream 의존 tool 호출 전 공통 `ensureDesktopReady()` 흐름을 거친다. 단,
  `ensureDesktopReady()` 는 미실행 상태에서 launch 명령을 내린 뒤 ready 까지 기다리지
  않는다. 이 경우 원 요청은 실행하지 않고 launch 요청 결과를 반환한다.
...
- 원 tool 요청은 `ensureDesktopReady()` 가 `running` 으로 통과한 경우에만 1회 실행한다.
  `not_running` / `stale_socket` 에서 launch 명령을 내린 경우에는 원 요청을 보관하거나
  이어서 실행하지 않는다. 사용자는 Desktop 이 뜬 뒤 같은 tool 을 다시 호출한다.
...
확인하면 NeoSQL Desktop detached launch 명령을 내리고, 원 요청은 실행하지 않은 채
launch 요청 결과를 반환한다.
...
- `running` 에서만 원 요청을 실행하고, launch 요청을 내린 경우에는 원 요청을 실행하지
  않는 control flow.
...
- 결과 상태: `launch_requested`, `not_installed`, `launch_failed` 등.
...
- detached launch 명령을 내린 경우 원 tool 요청은 실행하지 않고 `launch_requested` 계열
  응답을 반환한다.
