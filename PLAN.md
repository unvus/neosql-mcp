...
> **진행 방식**: Phase 0 → 1 → 2-1 ~ 2-4 까지 먼저 완성한다. Phase 2 는 embedded-server 의 Spring AI MCP 도구를 Node MCP 로 전면 이관하는 것이 목표이며, Sub-phase 2-1(인프라+시그니처+mock) → 2-2(분석+contract) → 2-3(Node 핸들러 일괄) → 2-4(SchemaTools real Electron pilot) 순으로 진행한다. Phase 3 이상은 Phase 2-4 완료 시점에 범위·우선순위를 다시 판단한다.
...
| Phase 2 전략          | **Option A + pilot** — Node 9개 핸들러 일괄 구현(mock UDS) → SchemaTools read-only real Electron pilot → 나머지 Electron 일괄(Phase 3+)                                                                             |
| 배포                  | **npm public registry** (`npx neosql-mcp`)                                                                                                                                                                          |

> 본체(electron-main / renderer) 변경은 Phase 2-4 pilot 시점에 첫 도입 (대상: SchemaTools). 그 외 도구의 본체 작업은 Phase 3+ 로 이월. HTTP 메서드 명세는 Phase 2-2 의 contract 문서로 확정한다 (socket path / HTTP path 는 본 문서의 상수 규칙을 따름).
...
| **2-4**   | 대표 read-only 도구(`SchemaTools` — `listTables`, `getTableDetails`) 로 real Electron pilot e2e                       | **첫 본체 PR** |
...
- Phase 2-4: 본체 PR 첫 도입. 대상은 `SchemaTools` — UDS listen + `chmod 0600` + stale unlink + dev/prod suffix + `/rpc` dispatcher + `listTables` / `getTableDetails` 구현.
...
- Phase 2-4 완료: SchemaTools 가 real electron-main 환경에서 e2e 통과.
...
- **나머지 카테고리 Electron 일괄 구현** — SQL → DDL → CodeGeneration 권장 순서(의존성·복잡도 오름차순). 카테고리 단위 PR + e2e 테스트.
