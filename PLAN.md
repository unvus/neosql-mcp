...
> **진행 방식**: Phase 0 → 1 → 2-1 ~ 2-4 까지 먼저 완성한다. Phase 2 는 embedded-server 의 Spring AI MCP 도구를 Node MCP 로 전면 이관하는 것이 목표이며, Sub-phase 2-1(인프라+시그니처+mock) → 2-2(분석+contract) → 2-3(Node 핸들러 일괄) → 2-4(본체 HTTP tool 구현 + as-is/to-be 비교 검증) 순으로 진행한다. Phase 3 은 Desktop lifecycle UX 를 구체화하고, 3-1(미실행 감지 UX) → 3-2(Desktop 실행 tool) → 3-3(미설치 감지·설치 안내) 순으로 진행한다.
...
MCP tool 흐름 안에서 진단하고 복구할 수 있게 한다. `neosql-mcp` 는 Electron app 을
직접 소유하지 않고, 실행 상태 확인·명시적 실행 요청·readiness polling·설치 안내까지만
담당한다.

### Phase 3 진행 원칙

- 기존 SQL/schema/DDL/codegen tool 호출에서 Desktop 이 미실행이면 자동 launch 하지 않는다.
  먼저 명확한 tool error/result 로 상태와 다음 행동을 알려준다.
- Desktop 실행은 별도 MCP tool 로 제공한다. 사용자가 명시적으로 실행 tool 을 호출한 경우에만
  OS 별 detached launch 를 시도한다.
- MCP client 가 안내를 반드시 따르는 것은 보장하지 않는다. tool description 과 error/result
  message 를 통해 실행 tool 호출을 유도하는 best-effort UX 계약으로 둔다.
- Phase 3-1 과 3-2 는 상태 enum/result envelope 을 먼저 합의하면 병렬 진행 가능하다.
  단, 3-3 의 정밀 미설치 감지는 OS 별 설치 위치·product name·app id 확인에 의존하므로
  3-2 의 최소 감지 이후 별도 확장한다.

### Phase 3-1 · 미실행 감지와 MCP UX

**목표**: 모든 upstream 의존 tool 에서 NeoSQL Desktop 미실행 상태를 일관되게 설명하고,
MCP client 가 실행 tool 을 제안하기 쉬운 응답을 제공한다.

산출물:

- `not_running`, `stale_socket`, `timeout`, `app_not_ready` 상태별 사용자-facing 메시지 정책.
- 기존 tool error mapping 정리. 예: "NeoSQL Desktop is not running. Call
  `launchNeoSqlDesktop` first." 형태의 다음 행동 안내.
- MCP host 별 수동 UX 확인 절차 (`docs/e2e-manual.md` 확장).

완료 조건:

- upstream 의존 tool 이 Desktop 미실행 상태에서 동일한 UX 메시지를 반환한다.
- stale POSIX socket 과 listener 부재가 사용자에게 같은 "Desktop 미실행" 범주로 보이되,
  로그/진단 정보에서는 구분 가능하다.
- MCP client 에게 실행 tool 호출을 권할 수 있는 tool description / result text 가 정리된다.

### Phase 3-2 · NeoSQL Desktop 실행 tool

**목표**: 사용자가 MCP client 를 통해 NeoSQL Desktop 실행을 명시적으로 요청할 수 있는
tool 을 제공한다.

산출물:

- `launchNeoSqlDesktop` MCP tool.
- OS 별 detached launch 모듈.
...
- launch 전 health check, launch 후 deterministic socket path readiness polling.
- 결과 상태: `already_running`, `launched`, `not_installed`, `launch_failed`,
  `not_ready_timeout` 등.

완료 조건:

- 이미 실행 중이면 중복 실행 없이 `already_running` 을 반환한다.
- 미실행이지만 설치되어 있으면 detached launch 후 readiness polling 성공 시 `launched` 를 반환한다.
- `neosql-mcp` 종료 시 Electron app 을 종료하지 않는다.
- MCP stdout(JSON-RPC) 과 launched app stdout/stderr 가 섞이지 않는다.
