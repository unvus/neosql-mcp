...
> **진행 방식**: Phase 0 → 1 → 2-1 ~ 2-4 까지 먼저 완성한다. Phase 2 는 embedded-server 의 Spring AI MCP 도구를 Node MCP 로 전면 이관하는 것이 목표이며, Sub-phase 2-1(인프라+시그니처+mock) → 2-2(분석+contract) → 2-3(Node 핸들러 일괄) → 2-4(ContextTools real Electron pilot) 순으로 진행한다. Phase 3 이상은 Phase 2-4 완료 시점에 범위·우선순위를 다시 판단한다.
...
| socket path 규칙 | POSIX: `${os.tmpdir()}/neosql-mcp${suffix}.sock` · Windows: `\\\\.\\pipe\\neosql-mcp${suffix}`. `suffix` = prod 빈 문자열 / dev `-dev`. mcp 는 `--dev` CLI 플래그로 profile 인지 (electron 의 dev 실행 규칙과 대칭) |
| HTTP path | `/rpc` 고정. 양쪽 코드의 상수로 보유 (config 미저장) |
| 미설치 vs 미실행 구분 | Phase 1 에서는 합친 메시지로 처리. 정밀 구분은 후속 Phase 에서 Supabase 등 데스크톱 앱 사례 참고하여 재검토 |
| Phase 순서 | Phase 0 → 1 → 2-1 ~ 2-4 선행, Phase 3 이상은 Phase 2-4 (Electron pilot) 완료 후 재검토 |
| Phase 2 전략 | **Option A + pilot** — Node 9개 핸들러 일괄 구현(mock UDS) → ContextTools 1개로 real Electron pilot → 나머지 Electron 일괄(Phase 3+) |
| 배포 | **npm public registry** (`npx neosql-mcp`) |

> 본체(electron-main / renderer) 변경은 Phase 2-4 pilot 시점에 첫 도입 (대상: ContextTools). 그 외 도구의 본체 작업은 Phase 3+ 로 이월. HTTP 메서드 명세는 Phase 2-2 의 contract 문서로 확정한다 (socket path / HTTP path 는 본 문서의 상수 규칙을 따름).
...
- `suffix` (profile 구분):
  - prod (npm 배포본): 빈 문자열
  - dev (로컬 개발): `-dev`
  - mcp 는 `--dev` CLI 플래그로 profile 인지. electron 도 dev 실행 시 동일 suffix 로 listen 한다는 전제 (본체 작업 시 확정).
...
| DDL | `DdlTools.java` | `createTables`, `alterTables` |
| SQL | `SqlTools.java` | `executeQuery` |

### 구현 전략 — Option A + pilot

Node 측 9개 핸들러를 일괄 구현(mock UDS 대상)한 뒤, 1개 대표 도구로 real Electron pilot 으로 contract 를 검증하고, 나머지 Electron 작업은 후속 Phase 에서 일괄 진행한다.

근거:
- Node 핸들러는 인자 검증 → upstream RPC 호출 → 응답 포매팅의 얇은 wrapper. 9개 일괄 구현 부담 적음.
- Electron 측은 HTTP 서버 부팅·공용 IPC·세션·에러 매핑 등 공유 인프라 비중이 커서 한두 PR 로 코히어런트하게 묶는 게 효율적.
...
| **2-2** | Java tool 5개 파일 분석 → 도구별 Node/Electron 분할 결정 + upstream RPC contract 문서 + 도구별 체크리스트 | 없음 |
| **2-3** | Node 핸들러 9개 실 구현 (Phase 2-2 contract 기반, mock UDS 대상 e2e green) | 없음 |
| **2-4** | 대표 도구 1개(권장: `ContextTools` — DB 의존 없음, 다른 도구의 의존성) 로 real Electron pilot e2e | **첫 본체 PR** |
...
- Phase 2-4: 본체 PR 첫 도입. 대상은 `ContextTools` 1개 — UDS listen + `chmod 0600` + stale unlink + dev/prod suffix + `/rpc` dispatcher + 3개 메서드 구현 + 필요 시 renderer IPC.

### 완료 조건

- Phase 2-1 완료: mock UDS 서버 환경에서 stdio 클라이언트가 9개 도구 호출 왕복 성공.
- Phase 2-2 완료: `docs/upstream-rpc-contract.md` + 도구별 체크리스트 작성.
- Phase 2-3 완료: 9개 Node 핸들러가 contract 기반으로 mock UDS 라운드트립 green.
- Phase 2-4 완료: ContextTools 가 real electron-main 환경에서 e2e 통과.
- Phase 2-4 종료 시점에 **Phase 3 이상 범위·우선순위 재검토** (나머지 Electron 카테고리 진행 순서·인증·multi-instance 등).

---

## Phase 3 이상 (Phase 2-4 완료 후 구체화)

Phase 2-4 pilot 결과를 확인한 뒤 아래 항목의 범위·우선순위·방식을 재검토한다.

- **나머지 카테고리 Electron 일괄 구현** — Schema → SQL → DDL → CodeGeneration 권장 순서(의존성·복잡도 오름차순). 카테고리 단위 PR + e2e 테스트.
