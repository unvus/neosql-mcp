...
- Supabase CLI 참조 분석 문서는 `docs/research/supabase-cli/`에 둔다. 분석 대상 원본
...
문서 작업 외 소스 코드 변경사항이 있는 작업을 완료하기 전에는 반드시 `npm run build`를
실행한다. 실행하지 못한 경우에는 그 이유를 최종 응답에 명확히 남긴다.
