...
- 초기 context 옵션(`--project`, `--connection`, `--schema`)의 상세 mapping과 MCP host 설정 예시는
...
6. 양쪽에 같은 context 를 넣는다.
   - to-be: `--project`, `--connection`, `--schema`
...
| `executeQuery`    | `{ "sql": "INSERT ..." }`                                                    | DML result                                           | 확인 완료                         |
| `createTables`    | `tableDefinitions`                                                           | ERD 생성 결과                                        | 확인 완료                         |
| `modifyTables`    | `alterations`                                                                | ERD 수정 결과                                        | 확인 완료                         |
...
2. profile 에 맞는 앱이 일반 설치 위치에 있는지 확인한다.
   ...
   - dev: `/Applications/NeoSQLDev.app` 또는 `~/Applications/NeoSQLDev.app`
3. MCP host 에서 `listTables` 같은 upstream 의존 tool 을 호출한다.
4. 기대 결과:
   - 응답 JSON 에 `status: "activation_requested"` 가 포함된다.
...
1. NeoSQL Desktop 이 다음 위치에 없는 상태를 만든다.
   ...
   - dev: `/Applications/NeoSQLDev.app`, `~/Applications/NeoSQLDev.app`
2. MCP host 에서 `listTables` 같은 upstream 의존 tool 을 호출한다.
3. 기대 결과:
