...
6. 양쪽에 같은 context 를 넣는다.
   - to-be: `--project`, `--connection`, `--schema`, `--ddl-execute`, `--auto-commit`
...
| ----------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------- | ---- |
| `setContext`      | `projectId`, `connectionId`, `schema`, `ddlExecute`, `autoCommit`             | 저장된 context 값                                     | 확인 완료 |
| `getContext`      | `{}`                                                                          | context JSON 구조와 값                                | 확인 완료 |
| `getContextHelp`  | `{}`                                                                          | stdio/npx 기준 도움말                                 | 확인 완료 |
| `listTables`      | `{ "schema": "<schema>" }`                                                     | table/view 목록, comment                              | 확인 완료 |
| `getTableDetails` | `{ "tableNames": ["<table>"], "schema": "<schema>" }`                         | columns/indexes/fks/constraints                       | 확인 완료 |
| `executeQuery`    | `{ "sql": "SELECT 1" }`                                                        | SELECT result JSON                                    | 확인 완료 |
| `executeQuery`    | `{ "sql": "INSERT ...", "autoCommit": false }`                                | transaction-open DML result                           | 확인 완료 |
| `executeQuery`    | `{ "sql": "INSERT ...", "autoCommit": true }`                                 | committed DML result                                  | 확인 완료 |
| `createTables`    | `tableDefinitions`, `executeImmediately` omitted/false/true                   | ERD 생성 및 DDL 실행 결과                             | 확인 완료 |
| `modifyTables`    | `alterations`, `executeImmediately` omitted/false/true                        | ERD 수정 및 DDL 실행 결과                             | 확인 완료 |
...
| ---- | ---- | ------- | ---- | ---------- | ------ | ---- |
| 2026-05-11 | automated mock UDS | prod/dev path independent | 9개 Node handler | contract fixtures | pass | `npm test` 기준, real Desktop 비교 아님 |
