...
현행 재현 절차·프로젝트 지정 검증·날짜별 결과는 본체의
[통합 런타임 검증](../../neosql/docs/manual-test/mcp-runtime-lifecycle.md)을 기준으로 한다.
아래 2026-09-15 체크와 실측은 당시 **20초** 기준의 기록이다. 절차의 기한은 현재 40초지만
기존 실측 수치를 40초 결과로 해석하지 않는다. C01~C09의 축소 합의 완료와 Windows·host UI·40초 경계 미검증을 구분한다.
...
기존 파일 logger를 사용한다. 앱과 일치하는 `--profile=local` 또는 `--profile=dev`로
진단 코드가 포함된 built CLI의 새 MCP 연결을 시작하면 debug 로그를 기록한다.
`stage`·`prod` 및 profile 생략 시에는 info 이상만 기록하며 `LOG_LEVEL`은 무시한다.
기존 실행 중 프로세스나
이미 배포된 npm 패키지에는 로컬 소스 변경이 자동 반영되지 않는다.

macOS/dev 기본 로그는 `~/Library/Logs/NeoSqlMcpDev/neosql-mcp.log`다.
`NEOSQL_MCP_LOG_PARENT_DIR`를 설정하면 해당 디렉터리 아래 profile별 로그를 사용한다.
stdout은 MCP 전용으로 유지하며 파일 생성 실패 시 기존 logger의 stderr fallback을 따른다.

`component=McpProgress`와 호출별 `traceId`로 다음을 연결한다.

- `preparation_started`: progressToken 유무·타입, upstream method, 서버 PID.
- `progress_skipped`: 관찰 단계와 `missing_progress_token`. 토큰이 없어서 알림을 보내지 않은 경우다.
- `progress_send_started` / `progress_send_completed` / `progress_send_failed`: 단계·progress·고정 안내문과 SDK 전송 함수의 시작/완료/실패.
- `preparation_finished`: 준비 종료 상태와 준비 경과 시간.
- `operation_started` / `operation_completed`: 원래 작업 시작/성공. 완료 경과 시간은 준비를 포함한 호출 전체 시간이다.

새 진단 이벤트에는 토큰 원문·세션 ID·도구 인자·SQL·연결 정보·결과 원문·전송 오류 원문을
기록하지 않는다. traceId는 진단용 임의 ID이며 host JSON-RPC request ID가 아니다.
`progress_send_completed`는 SDK 전송 함수 성공을 뜻하며 host 수신·화면 표시·모델 입력 전달을
보장하지 않는다. 취소로 중간에 끝난 경우 준비/작업 완료 이벤트가 없을 수 있다.
profile은 연결 대상 앱도 결정하므로 로그만 켜기 위해 다른 앱의 profile로 바꾸지 않는다.

### 이번 변경의 목적과 실행 체크리스트 — macOS / dev

이번 변경은 한 번의 도구 호출 안에서 앱/프로젝트 준비를 기다리며
`notifications/progress`로 현재 단계를 클라이언트에 알리고, 준비되면 원래 작업을
한 번 실행하는 흐름이다. 상태 조회마다 반복 알림을 보내지 않는다.
앱 실행 요청 → 앱 연결/Renderer 준비 → 프로젝트 loading → ready는 실제 관찰된 단계만
알리며, 모든 호출에서 모든 단계가 나타날 필요는 없다.
프로젝트 미선택·사용자 조치 필요는 자동 대기를 계속하지 않고 최종 안내로 종료한다.

검증 층을 구분한다: **서버 전송 → SDK/host 수신 → 실제 에이전트에 중간 전달 또는 UI 표시**.
SDK 수신이나 assistant가 직접 작성한 진행 설명은 실제 host의 중간 전달 증거가 아니다.
UI 표시와 에이전트가 메시지를 받는 것도 별개로 기록한다. 2026-09-15 사용자 합의로
이번 검증의 완료 조건은 서버 측 전송까지다. host UI 표시·에이전트 입력 전달은 통제 범위
밖의 참고 관찰이며 필수 완료 조건으로 삼지 않는다.

앞으로 실행할 항목은 아래 목적 중심 목록을 기준으로 한다. 기존 분기별 결과는 아래에
보존하지만, 미체크 상세 분기를 모두 수동 재현하는 것을 이번 E2E의 완료 조건으로 삼지 않는다.
드라이버·플랜 정리·초기화 실패 사유 전종류 재현보다 대표 흐름과 알림 전달 경로를 우선한다.

- [x] **C01 앱 기동 중 단계 알림** — I05 실제 SDK wire에서 실행 요청 → 실행 요청 완료 → Renderer 준비 대기 알림이 최종 결과 전에 도착했다. 동일 토큰·증가 progress·total 생략을 재확인했다. 첫 호출은 프로젝트 미선택으로 정상 종료했다.
- [x] **C02 프로젝트 대기 → 준비 → 작업 완료** — P04에서 최초 loading, 약 10.3초 뒤 ready 알림, 동일 호출의 작업 1회·정상 최종 결과를 확인했다. 21회 준비 조회에 loading/ready 알림은 각 1회였다. SDK 수신 기준이다.
- [x] **C03 준비 기한 초과로 종료** — I07/P05에서 약 20초 후 readiness_timeout·작업 0회, 일시 정지 해제 후 앱 회복 확인. 알림이 무제한 대기를 만드는지 확인하는 대표 경계다. 인위적 지연 조건은 기존 기록을 따른다.
- [x] **C04 사용자 조치 필요 시 종료·해결 후 새 호출** — A01~A04 잠금 대표 사례에서 조치 안내로 종료·작업 0회, 해제 후 새 호출 정상 처리 확인. 모든 조치 사유의 개별 수동 재현은 요구하지 않는다.
- [x] **C05 실제 host 호출의 서버 중간 알림 전송** — 2026-09-15 17:28~17:29 KST, 실제 host의 숫자 progressToken을 확인했다. Dev 앱 자동 실행 후 약 8.2초 일시 정지·자동 재개 조건에서 activation_requesting → activation_requested → renderer_loading의 `progress_send_completed`를 확인했고, 동일 호출은 10,833ms 후 `project_not_selected`, `requestSent: false`로 종료했다. 완료 후 host 캡처에는 최종 결과만 보였다. 사용자 합의로 완료 조건은 서버 전송까지이며 host UI 표시·에이전트 입력 전달은 완료 조건에서 제외한다. 해당 층의 성공을 주장하지 않는다.
- [x] **C06 progressToken 호환성** — 기존 자동 검증 T21로 확인했다. built CLI + 실제 stdio + mock Desktop UDS에서 토큰 없음/숫자 0/문자열 각각 loading → ready → 원래 작업 1회·최종 응답 1개를 확인했다. 0과 문자열은 동일 토큰의 loading/ready 알림을 받고, 토큰 없음은 알림 0개다. 2026-09-15 16:02 실행 로그 `/tmp/neosql-progress-tests.log`에서 세 변형 통과(전체 240/240). 사용자 합의로 이 근거를 재사용하며 실제 Desktop에서 토큰 변형을 재실행한 것으로 기록하지 않는다.
- [x] **C07 대기 중 취소·연결 종료** — 사용자 합의로 기존 자동 검증 T18과 built CLI EOF 회귀 결과를 재사용했다. SDK notifications/cancelled 및 client.close에서 pending HTTP 종료, stdin EOF 후 새 조회·알림·원래 작업 없음과 CLI 정상 자연 종료를 확인했다. 실행 후 앱 유지 정책은 자동 검증 근거이며 실제 Desktop 유지·host 취소 버튼 전달을 새로 관찰한 것은 아니다. (N05~N08)
- [x] **C08 준비 시간과 작업 제한 분리** — 기존 T09 자동 검증과 I07/P05 실제 약 20초 종료 측정을 재사용한다. 알림·프로젝트 변경으로 준비 기한이 연장되지 않으며 ready 반환 뒤에도 deadline을 확인한다. 준비 성공 후 shared.ts가 기존 timeoutMs를 postRpc에 전달하는 코드 경로를 확인했다. 장시간 실제 DB 작업은 추가 실행하지 않았다. (N09)
- [x] **C09 대기 중 일시 무응답 후 회복** — 기존 자동 테스트 T07의 loading → HTTP timeout → IPC timeout → ready 회복 통과 근거를 재사용한다. ready 이후 작업 1회·최종 응답은 기존 C02/T21 근거와 함께 확인한다. 사용자 합의에 따라 추가 수동 재현 없이 완료 처리한다. (Q04)

**C01~C09 검증 완료.** 실제 실행 결과와 기존 자동 검증·코드 확인 근거를 함께 사용하는
합의된 기준으로 완료했다. 서버 중간 알림 전송이 이번 검증의 범위이며 host UI 표시·
에이전트 입력 전달은 필수 완료 조건에서 제외한다. 추가 수동 테스트는 없다.

### 참고: 상세 분기별 체크리스트와 실행 이력 — macOS / dev

아래는 기존 상세 분기와 실행 이력이다. 앞으로의 실행 범위·순서는 위 C01~C09를 따른다.
미체크 항목은 개별 동작의 미확인을 뜻하며 자동으로 추가 수동 작업이 되지 않는다.
기존 M01~M11은 참고 시나리오로 보존한다. 괄호의 M 번호는 원래 절차와의 대응 관계다.
...
- [x] **P03 이미 실행 중·프로젝트 미선택** — 기존 실행 앱의 미선택 상태에서 `project_not_selected`, 원래 작업 0회, OS 실행 명령 0회를 확인한다. (M01의 미선택 분기) **완료:** 2026-09-15 13:04 KST, 실행 중인 Dev PID 3217의 responsive/not_selected를 확인한 뒤 SDK 도구 호출 1회. 약 14ms 후 `project_not_selected`·`requestSent: false`, 준비 조회 1회·원래 작업 전송 0회·OS 실행 명령 0회를 계측했다. 실제 host UI와는 구분한다.
- [x] **P04 프로젝트 로딩 중 → 준비 완료** — 최초 상태 조회에서 실제 loading을 확인하고 기한 내 ready이면 추가 호출 없이 원래 작업 1회·기준 결과를 반환한다. 첫 조회부터 ready인 실행은 이 행에 체크하지 않는다. (M02) **완료:** 2026-09-15 13:15 KST, 사용자 프로젝트 선택 후 SDK 도구 호출의 최초 조회부터 `loading`을 확인했다. 준비 조회 21회 후 `ready`, 같은 호출에서 원래 작업 1회·연결 목록 2개 정상 반환(총 10,280ms), OS 실행 명령 0회. 인위적 지연 없이 실제 초기화를 관찰했다. 사전 기준 목록과의 항목별 비교는 별도 미검증이다.
- [x] **P05 프로젝트 로딩 중 → 준비 시간 초과** — 실제 loading이 전체 준비 기한을 넘으면 `readiness_timeout`, 원래 작업 0회로 종료한다. (M06·M11) **완료(loading 확인 후 일시 정지 변형):** 2026-09-15 14:09 KST, 최초 도구 조회에서 실제 `loading` 확인 후 Dev main을 SIGSTOP했다. 20,010ms 후 `readiness_timeout`·`requestSent: false`, 원래 작업 0회·OS 실행 명령 0회. 자동 SIGCONT 후 동일 PID가 `ready`로 회복했다. 정지 중에는 상태 조회가 응답하지 않으므로, 계속 loading을 응답하는 자연적인 장시간 초기화 변형은 미검증이다.
- [x] **P06 이미 준비된 local 프로젝트·익명 세션** — OS 실행 명령 없이 원래 작업 1회 성공하고 로그인 안내가 없다. `local` 프로젝트 유형과 앱의 `dev` 빌드 profile은 별개다. (M09) **완료:** 2026-09-15 14:22 KST, 사용자가 로그아웃·local 프로젝트·연결 미등록을 확인했다. SDK 최초 준비 조회 `ready`, 도구 1회·원래 작업 1회·OS 실행 명령 0회, 약 37ms 후 빈 연결 목록 정상 반환. 로그인 요구 응답 없음. 세션·프로젝트 유형은 사용자 확인, 준비 상태·응답·횟수는 계측 근거다.
- [x] **P07 이미 준비된 account 프로젝트·로그인 세션** — OS 실행 명령 없이 원래 작업 1회·정상 결과를 반환한다. (M09) **완료:** 2026-09-15 14:35 KST, 사용자가 로그인·계정 프로젝트 진입을 확인했다. SDK 최초 준비 조회 `ready`, 도구 1회·원래 작업 1회·OS 실행 명령 0회, 약 24ms 후 연결 목록 2개 정상 반환. 세션·프로젝트 유형은 사용자 확인, 준비 상태·응답·횟수는 계측 근거다.
...
- [x] **A01 프로젝트 진입 시 잠금** — `unlock_project` 사유의 대기 안내를 확인한다. **완료:** 2026-09-15 14:54 KST, 사용자 잠금 설정·세션 잠금 후 대시보드를 경유해 재진입한 해제 창에서 사전 내부 상태 `user_action_required`·`unlock_project` 확인. SDK 호출 1회, 약 11ms 후 잠금 해제 후 재호출 안내·`requestSent: false`, 원래 작업 0회·OS 실행 명령 0회.
- [x] **A02 진입 잠금 해제 후 새 호출** — A01의 잠금을 해제한 뒤 현재 상태로 진행한다. **완료:** 2026-09-15 14:59 KST, A01 종료 후 사용자가 잠금 해제·진입 완료. 새 SDK 호출의 최초 상태 `ready`, 약 21ms 후 빈 연결 목록 정상 반환, 원래 작업 1회·OS 실행 명령 0회. 이전 잠금 안내 없음. 별도 MCP 프로세스의 새 호출이며 동일 transport 재호출은 별도다.
- [x] **A03 이미 로드된 프로젝트의 자동 잠금** — 실제 자동 잠금 후 `unlock_project` 사유의 대기 안내를 확인한다. **완료:** 2026-09-15 15:05 KST, 사용자가 자동 잠금 창 발생을 확인했다. 사전 내부 상태 `user_action_required`·`unlock_project`, SDK 호출 1회·약 16ms 후 잠금 해제 후 재호출 안내·`requestSent: false`, 원래 작업 0회·OS 실행 명령 0회. 자동 발동은 사용자 관찰, 상태·응답·횟수는 직접 계측 근거다.
- [x] **A04 자동 잠금 해제 후 새 호출** — A03의 잠금을 해제한 뒤 현재 상태로 진행한다. **완료:** 2026-09-15 15:07 KST, A03 종료 후 사용자가 자동 잠금을 해제했다. 새 SDK 호출 최초 상태 `ready`, 약 26ms 후 빈 연결 목록 정상 반환, 이전 잠금 안내 없음·원래 작업 1회·OS 실행 명령 0회. 별도 MCP 프로세스의 새 호출이다.
...
- [ ] **N09 준비 제한과 작업 제한 분리** — 준비 구간·작업 구간·host 전체 시간을 각각 측정하고, 준비의 40초 기한과 기존 도구 실행 제한이 분리됨을 확인한다. (M06)
...
- [ ] **S06 프로젝트 전환으로 기한 초기화 안 됨** — 전환 전후 전체 준비 기한이 최초 확인 기준 40초로 유지된다. (M10)
...
체크 완료한 I01·I03·I04·I05·I06·I07·I08·P01·P02·P03·P04·P05·P06·P07·A01·A02·A03·A04의 결과·미확인 증거는
...
| M06 | 준비 지연과 실제 작업 시간을 측정할 수 있는 테스트 프로젝트 | 읽기 도구를 호출해 준비 구간·작업 구간·host 전체 제한을 따로 측정 | 준비 40초와 기존 도구 실행 제한이 분리됨. host가 먼저 중단하면 해당 제한과 미완료 구간을 기록. 알림이 host 제한을 늘린다고 가정하지 않음 |
...
| M09 | 앱·프로젝트가 이미 준비됨. local 익명과 account 로그인 환경 각각 준비 | 도구 1회씩 호출 | 앱 실행 요청 없이 작업 1회. local은 로그인 안내 없음. |
...
구현 완료 후 각 시나리오·OS·변형별로 아래 내용을 채운다. 상세 실행 방법과 결과는 외부 MCP의 기존 `docs/e2e-manual.md`에 반영하고, 통합 검증 문서에는 결과 위치와 완료 여부를 연결한다. 실제 재현 과정에서 동작 정책 변경이 필요하면 즉시 멈추고 사용자에게 알린다.
...
현재 전체 준비 제한은 40초다. 위 C03/C08의 약 20초 측정은 변경 전 실제 실행 기록이며 보존한다. 40초 경계는 자동 테스트로 검증하고, 실제 Desktop의 지연·시간 초과 재측정은 미실행이다.
