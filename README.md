...
neosql MCP 개발 실습 공간. 클라이언트(MCP Host) 와 neosql Electron 앱 사이를 중계하는 npx 기반 MCP 서버 `neosql-mcp` 를 설계·구현한다.
...
- electron-main 의 socket path 는 사용자 환경에 따라 결정되며, 클라이언트가 고정 식별자로 접근할 수 없다 (config 파일을 읽어야 함).
...
- POC 에서 핵심 시나리오 (POST/JSON, SSE, N:1 multi-connection) 실측 통과 (`poc/` 디렉토리, gitignore).
...
- 실행 중인 neosql Electron 앱의 **현재 엔드포인트** (socket path · http path) 를 config 파일로부터 확인한다.
- 해당 엔드포인트로 도구 핸들러의 RPC 호출을 위임한다 (`http.request({ socketPath, ... })`).
